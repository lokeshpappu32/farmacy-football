  ...
  return (
    <div className="space-y-6">
      {(showIdentity ?? mode === "rep") && (
        <IdentityHeader nameLabel={t("identity.participantName", "Participant name")} rank={ownStanding?.rank} points={ownStanding?.total_points} />
      )}
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          <h1 className="text-3xl font-black">{title || (mode === "rep" ? t("navigation.myStanding", "My Standing") : "HETERO Staff Standing")}</h1>
      ...
      {mode === "rep" && !loading && !error && (
        <div className="grid gap-6 xl:grid-cols-2">
          <section className="glass rounded-3xl p-5 md:p-6">
            <h2 className="text-xl font-black">{t("performance.myPointsHistory", "My Points History")}</h2>
            <div className="scroll-panel mt-4 max-h-[360px] space-y-3 overflow-y-auto pr-2">
              ...
              {(data?.points_history || []).length === 0 && <div className="rounded-2xl bg-white/10 p-4 text-white/60">{t("performance.noPointsHistory", "No points history yet.")}</div>}
            </div>
          </section>

          <section className="glass rounded-3xl p-5 md:p-6">
            <h2 className="text-xl font-black">{t("performance.myMatchHistory", "My Match History")}</h2>
            <div className="scroll-panel mt-4 max-h-[360px] space-y-3 overflow-y-auto pr-2">
              {(data?.match_history || []).map((item) => (
                <div key={item.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="font-black">{item.match?.team1} vs {item.match?.team2}</div>
                  <div className="mt-1 text-sm text-white/65">{t("performance.matchPrediction", "Prediction:")} <span className="font-bold text-gold">{item.predicted_team === "Draw" ? t("common.draw", "Draw") : item.predicted_team}</span></div>
                  <div className="mt-1 text-xs text-white/50">{item.is_correct === true ? t("common.correct", "Correct") : item.is_correct === false ? t("performance.wrongNoBonus", "Wrong / No bonus") : t("performance.pendingResult", "Pending result")}</div>
                </div>
              ))}
              {(data?.match_history || []).length === 0 && <div className="rounded-2xl bg-white/10 p-4 text-white/60">{t("performance.noMatchHistory", "No match history yet.")}</div>}
            </div>
          </section>
        </div>
      )}
